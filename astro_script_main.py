...
def format_output(event_data: dict, chart_data: dict, args: dict) -> str:
    ...
    result += format_event_info(event_data, output_type)

    # Planet positions
    result += f"{fmt['h2']}Planetary Positions{fmt['h2_']}\n"
    for planet, position in chart_data["planet_positions"].items():
        if planet not in ["house_cusps"]:
            house = chart_data["house_positions"].get(planet)
            formatted_pos = format_planet_position(
                planet,
                position["longitude"],
                house,
                position.get("retrograde", False),
                output_type,
            )
            result += f"{formatted_pos}\n"
    result += "\n"

    # Aspects
    if chart_data["aspects"]:
        result += format_aspect_table(
            chart_data["aspects"],
            chart_data["planet_positions"],
            output_type,
            args.get("Aspect Scores", False),
            "Natal",
        )
        result += "\n"

    # House positions
    result += format_house_positions(
        chart_data["planet_positions"], chart_data["house_positions"], output_type
    )
    result += "\n"
    # Chart patterns
    if chart_data["patterns"]:
        result += format_chart_patterns(chart_data["patterns"], output_type)
        result += "\n"

    # Fixed star aspects
    if chart_data.get("star_aspects"):
        result += format_fixed_star_aspects(chart_data["star_aspects"], output_type)
        result += "\n"
    ...
    # Moon phase
    if chart_data.get("moon_phase"):
        result += format_moon_phase(chart_data["moon_phase"], output_type)
        result += "\n"

    # Numerology
    if args.get("Numerology") and event_data.get("name"):
        numerology_data = calculate_all_numerology(
            event_data["name"], event_data["datetime"]
        )
        result += format_numerology(numerology_data, output_type)
...
def main(gui_arguments=None):
    ...
    try:
        ...
        output = format_output(event_data, chart_data, args)

        # Print to console unless it's HTML or return_html
        if args.get("Output") not in ("html", "return_html"):
            print(output)
