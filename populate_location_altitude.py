...
cursor.execute("SELECT id, latitude, longitude FROM myapp_location WHERE altitude IS NULL")
...
for row in rows:
    ...
    if altitude is not None:
        cursor.execute("UPDATE myapp_location SET altitude = ? WHERE id = ?", (altitude, id))
